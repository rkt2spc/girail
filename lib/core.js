...
exports.assignMessageProject = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        //-------------------------        
        // Temporary mapping selection to coupe with current app bug, correct above
        var mapping = undefined;
        message.receivers.find((receiver) => {
            mapping = mailbox.project_mappings.find((map) => map.receivers.includes(receiver));
        ...
        async.eachSeries(message.attachments, (attachment, cb) => {
            message.mailbox.getAttachment({
                ...
                jira.uploadAttachment({
                    ...
                    if (err) logger.error(`Attachment Upload Failed ${attachment.filename} (Project: ${message.project.key}, Issue: ${message.issueId}, Info: ${err.message})`);
                    else logger.info(`Attachment Uploaded ${attachment.filename}`);
                    cb(null);
                });
            });
        }, (err) => {
