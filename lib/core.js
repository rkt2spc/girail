    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        if (!message.mailbox)
            return reject(new DropSignal({
                code: 'DS001',
                message: 'Invalid message, no mailbox property', 
                data: { 
        ...
        if (!mailbox)
            return reject(new DropSignal({
                code: 'DS002',
                message: `Invalid message, invalid mailbox ${message.mailbox}`,
                data: { 
                    messageId: message.id
                }
            }));
        ...
        message.mailbox.getMessage(message.id, (err, details) => {
            if (err)
                return reject(new RecoverableError({ 
                    code: 'RE001', 
                    message: 'Gmail Failure: Getting message details',
                    src: err,
                    data: {
                        messageId: message.id,
...
exports.formatMessage = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        //-------------------------
        // Normalizing message
        if (!transformedMessage.subject || transformedMessage.subject.trim().length === 0)
            transformedMessage.subject = 'Untitled';
        else
            transformedMessage.subject = transformedMessage.subject.trim();
        
        if (!transformedMessage.content || transformedMessage.content.trim().length === 0)
            transformedMessage.content = 'No content';
        else
            transformedMessage.content = transformedMessage.content.trim();
...
exports.checkMessage = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        if (mailbox.whitelist && lodash.intersection(mailbox.whitelist, message.senders).length < message.senders.length)
            return reject(new DropSignal({ 
                code: 'DS003', 
                message: 'Message was from unauthorized sender, un-whitelisted', 
                data: {
                    messageId: message.id,
                    mailbox: message.mailbox.name,
        ...
        if (mailbox.blacklist && lodash.intersection(mailbox.blacklist, message.senders).length > 0)
            return reject(new DropSignal({ 
                code: 'DS004', 
                message: 'Message was from unauthorized sender, blacklisted',
                data: {
                    messageId: message.id,
                    mailbox: message.mailbox.name,
                    senders: message.senders
        ...
        // Message either a reply or a forwarded message
            ...
            message.mailbox.getThread(message.threadId, (err, thread) => {
                if (err)
                    return reject(new RecoverableError({ 
                        code: 'RE002', 
                        message: 'Gmail Failure: Getting message thread', 
                        src: err,
                        data: {
                            messageId: message.id,
                            threadId: message.threadId,
...
exports.assignMessageProject = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        //-------------------------        
        // Temporary mapping selection to coupe with current app bug, correct one above
        var mapping = undefined;
        for (let i = 0; i < message.receivers.length; ++i) {
            mapping = mailbox.project_mappings.find((pmap) => pmap.receivers.includes(message.receivers[i]));
            if (mapping) break;
        }
        ...
        if (!mapping)
            return reject(new DropSignal({ 
                code: 'DS005', 
                message: 'Message doesn\'t have valid receivers',
                data: {
                    messageId: message.id,
                    mailbox: message.mailbox.name,
        ...
        database
            ...
            .findDuplicatedMapping(message, (err, mapping) => {
                if (err)
                    return reject(new RecoverableError({ 
                        code: 'RE003', 
                        message: 'Database Failure: Finding duplicated mapping', 
                        src: err,
                        data: {
                            messageId: message.id,
                            mailbox: message.mailbox.name
                ...
                if (mapping)
                    return reject(new DropSignal({
                        code: 'DS006', 
                        message: 'Message was duplicated',
                        data: {
                            messageId: message.id,
                            mailbox: message.mailbox.name,
            ...
            .createMapping(message, (err, mapping) => {
                ...
                if (err && err.code !== 11000) // Natural disaster (Network failure blah blah)
                    reject(new RecoverableError({ 
                        code: 'RE004', 
                        message: 'Database Failure: Creating message mapping', 
                        src: err,
                        data: {
                            messageId: message.id,
                            mailbox: message.mailbox.name
                    ...
                    database
                        ...
                        .getMapping(message, (err, mapping) => {
                            ...
                            if (err) // Another Natural disaster ^^
                                return reject(new RecoverableError({ 
                                    code: 'RE005', 
                                    message: 'Database Failure: Getting message mapping', 
                                    src: err,
                                    data: {
                                        messageId: message.id,
                                        mailbox: message.mailbox.name
    ...
    var promise = new Promise((fulfill, reject) => {
        jira.createIssue(message, (err, issue) => {
            if (err)
                return reject(new RecoverableError({
                    code: 'RE006',
                    message: 'Jira Failure: Creating Jira Issue',
                    src: err,
                    data: {
                        messageId: message.id,
                        mailbox: message.mailbox.name
        ...
        database
            ...
            .findReplySourceMapping(message, (err, mapping) => {
                if (err)
                    return reject(new RecoverableError({ 
                        code: 'RE007', 
                        message: 'Database Failure: Finding reply source mapping', 
                        src: err,
                        data: {
                            messageId: message.id,
                            mailbox: message.mailbox.name
                ...
                jira.createComment(message, (err, comment) => {
                    if (err)
                        return reject(new RecoverableError({ 
                            code: 'RE008', 
                            message: 'Jira Failure: Creating Jira Comment', 
                            src: err,
                            data: {
                                messageId: message.id,
                                mailbox: message.mailbox.name
            ...
            .updateMapping(message, (err, mapping) => {
                if (err)
                    return reject(new UnrecoverableError({
                        code: 'UE001',
                        message: 'Database Failure: Updating mapping after creating Jira Entity',
                        src: err,
                        data: { 
                            messageId: message.id,
                            mailbox: message.mailbox.name,
            ...
            message.mailbox.getAttachment({
                ...
                if (err) {
                    logger.error(new UnrecoverableError({
                        code: 'UE002',
                        message: `Attachment Download Failed ${attachment.filename}`,
                        src: err,                        
                        data: {
                            messageId: message.id,
                            mailbox: message.mailbox.name,
                ...
                jira.uploadAttachment({
                    ...
                    if (err) {
                        logger.error(new UnrecoverableError({
                            code: 'UE003',
                            message: `Attachment Upload Failed ${attachment.filename}`,
                            src: err,
                            data: {
                                messageId: message.id,
                                mailbox: message.mailbox.name,
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        message.mailbox.markMessageProcessed(message, (err) => {
            if (err)
                return reject(new UnrecoverableError({
                    code: 'UE004',
                    message: 'Gmail Failure: Marking message processed',
                    src: err,
                    data: { 
                        messageId: message.id,
                        mailbox: message.mailbox.name
