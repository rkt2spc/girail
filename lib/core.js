...
exports.assignMessageProject = (message, callback) => {
  ...
  const promise = new Promise((fulfill, reject) => {
    ...
    const project = mailbox.projects.find((proj) => lodash.intersection(message.receivers, proj.receivers).length >= 1);
...
exports.extractMessageMetadata = (message, callback) => {
  ...
  const promise = new Promise((fulfill, reject) => {
    ...
    //-------------------------
    let metadata = message.content.match(/_+ *\r?\n(.+\r?\n?)+/g);
    metadata = lodash
      .chain(metadata)
...
exports.createJiraEntity = (message, callback) => {
  if (message.type === 'reply') return utils.wrapAPI(exports.createJiraComment(message), callback);
...
exports.sendReplyOnIssueCreation = (message, callback) => {
  ...
  const promise = new Promise((fulfill, reject) => {
    ...
    //-------------------------
    let mailTargets = lodash
      .chain(message.receivers)
      .concat(message.senders)
      .without(message.mailbox.name)
      .value();

    if (['MSF', 'MFA'].includes(message.project.key)) {
      mailTargets = lodash.concat(message.senders, 'dungnguyen@fossil.com');
    }
    ...
    message
      ...
      .sendMessage({
        ...
        threadId   : message.threadId,
        html       : `Thank you for your feedback. Ticket has been created at <a href="https://misfit.jira.com/projects/${message.project.key}/issues/${message.issueKey}">https://misfit.jira.com/projects/${message.project.key}/issues/${message.issueKey}</a>`, // eslint-disable-line max-len
      },
      (err) => {
        if (err) {
          logger.error(new UnrecoverableError({
