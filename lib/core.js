...
exports.formatMessage = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        //-------------------------
        // Normalizing message
        if (!transformedMessage.subject || transformedMessage.subject.length === 0 || transformedMessage.subject.trim().length === 0)
            transformedMessage.subject = 'Untitled';
        else
            transformedMessage.subject = transformedMessage.subject.trim();
        
        if (!transformedMessage.content || transformedMessage.content.length === 0 || transformedMessage.content.trim().length === 0)
