...
exports.formatMessage = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        // Format receiver
        if (headers['To'])
            transformedMessage.receiver = headers['To'].match(/<(.+@.+)>/g)[0]
                .replace(/[<>]/g, '');
...
exports.checkMessage = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        //-------------------------        
        if (mailbox.whitelist && !mailbox.whitelist.includes(message.sender))
            return reject(new DropSignal({ code: 'DS003', message: 'Message was from unauthorized sender, un-whitelisted' }));

        //-------------------------        
        if (mailbox.blacklist && mailbox.blacklist.includes(message.sender))
...
exports.assignMessageProject = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        //-------------------------
        var mapping = mailbox.project_mappings.find((map) => lodash.intersection(message.cc, map.cc).length === map.cc.length);
        if (!mapping)
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        database
            ...
            .updateMapping(message, (err, mapping) => {
                if (err)
                    return reject(new UnrecoverableError({ code: 'UE001', message: 'Database Failure: Updating mapping after creating Jira Entity', src: err }));
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        async.eachSeries(message.attachments, (attachment, cb) => {
            message.mailbox.getAttachment({
                ...
                if (err)
                    return cb(new UnrecoverableError({ code: 'UE002', message: 'Gmail Failure: Failed to download attachment', src: err }));
                ...
                jira.uploadAttachment({
                    ...
                    if (err)
                        return cb(new UnrecoverableError({ code: 'UE003', message: 'Jira Failure: Failed to upload attachment', src: err }));
...
exports.markMessageProcessed = function (message, callback) {
    ...
    var promise = new Promise((fulfill, reject) => {
        ...
        message.mailbox.markMessageProcessed(message, (err) => {
            if (err)
                return reject(new UnrecoverableError({ code: 'UE004', message: 'Gmail Failure: Marking message processed', src: err }));
            fulfill(message);
        });
    });
